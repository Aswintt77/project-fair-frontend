                    ...
                    {allUserProject.length > 0 ? (
                        allUserProject.map((item, index) => (
                            <div className='d-inline-flex justify-content-between mt-3'>
                                <h5 className='text-center ms-4'>{item.title}</h5>
                                <div className=' '>
                                <Edit project={item}/> 
                                    <a href={item?.github} target="_blank" ><button className='btn btn-secondary m-1'><i className='fa-brands fa-github'></i></button></a>
                                    <button className='btn btn-danger m-1' onClick={() => deleteProject(item?._id)}><i class="fa-solid fa-trash"></i></button>
                                </div >
                            </div>
                        ))
                    ) : (
